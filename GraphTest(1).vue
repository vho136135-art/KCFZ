  ...
  <div style="width: 100%;height: 100%;">
    ...
    <div class="content">
      ...
      <div
        ...
        <!-- // -->
        <div class="graphcontains2">
          <div class="custom-select" @click="showOptions33">
            ...
            <div style="width: 10vw;position: relative;">
              ...
              <img src="../assets/img/graphDown.png"
                style="position: absolute;z-index: 99;width: 5%;height: 100%;object-fit: contain;right: 0.5vw;top: 0;"
                @click="toggleOptions3" v-if="typePic3 == 0">
            ...
            <div style="width: 100%;height: 100%;">
              <div class="gContainer">
                <RelationGraph ref="graphRef" :options="graphOptions" :on-node-expand="onNodeExpand"
                  ...
                  <template #node="{ node }">
                    <div v-if="node.data.type == 0 && node.data.group == '1'" style="font-size: 2vw;padding-top: 40%;"  @dblclick="showPopup(node)">
                      {{ node.text }}</div>
                    <div v-if="node.data.type == 0 && node.data.group == '2'" style="font-size: 1.8vw;padding-top: 40%;"  @dblclick="showPopup(node)">
                      {{ node.text }}</div>
                    <div v-if="node.data.type == 0 && node.data.group == '3'" style="font-size: 1.8vw;padding-top: 40%;"  @dblclick="showPopup(node)">
                      {{ node.text }}</div>
                    <div v-if="node.data.type == 0 && node.data.group == '4'" style="font-size: 1.8vw;padding-top: 40%;"  @dblclick="showPopup(node)">
                      {{ node.text }}</div>
                    <div v-if="node.data.type == 0 && node.data.group == '5'" style="font-size: 1.8vw;padding-top: 40%;"  @dblclick="showPopup(node)">
                      {{ node.text }}</div>
                    <div v-if="node.data.type == 0 && node.data.group == '6'" style="font-size: 1.8vw;padding-top: 40%;"  @dblclick="showPopup(node)">
                      {{ node.text }}</div>
                    <div v-if="node.data.type == 0 && node.data.group == '7'" style="font-size: 1.8vw;padding-top: 40%;"  @dblclick="showPopup(node)">
                      {{ node.text }}</div>




                    <div v-if="node.data.type == 1 && node.data.group == '1'" style="font-size: 1.5vw;padding-top: 20%;"
                      @dblclick="showPopup(node)">{{ node.text }}</div>
                    <div v-if="node.data.type == 1 && node.data.group == '2'" style="font-size: 1.5vw;padding-top: 20%;"
                      @dblclick="showPopup(node)">{{ node.text }}</div>
                    <div v-if="node.data.type == 1 && node.data.group == '3'" style="font-size: 1.5vw;padding-top: 20%;"
                      @dblclick="showPopup(node)">{{ node.text }}</div>
                    <div v-if="node.data.type == 1 && node.data.group == '4'" style="font-size: 1.5vw;padding-top: 20%;"
                      @dblclick="showPopup(node)">{{ node.text }}</div>
                    <div v-if="node.data.type == 1 && node.data.group == '5'" style="font-size: 1.5vw;padding-top: 20%;"
                      @dblclick="showPopup(node)">{{ node.text }}</div>
                    <div v-if="node.data.type == 1 && node.data.group == '6'" style="font-size: 1.5vw;padding-top: 20%;"
                      @dblclick="showPopup(node)">{{ node.text }}</div>
                    <div v-if="node.data.type == 1 && node.data.group == '7'" style="font-size: 1.5vw;padding-top: 20%;"
                      @dblclick="showPopup(node)">{{ node.text }}</div>




                    <div v-if="node.data.type == 2"
                      :style="{ 'background-image': 'url(' + 'http://127.0.0.1:10035/profile/videoPic.png' + ')' }"></div>
                    <div v-if="node.data.type == 3"
                      :style="{ 'background-image': 'url(' + 'http://127.0.0.1:10035/profile/dhvideoPic.png' + ')' }"></div>
                    <div v-if="node.data.type == 3"
                      :style="{ 'background-image': 'url(' + 'http://127.0.0.1:10035/profile/jsq.png' + ')' }"></div>
                  </template>
                </RelationGraph>
  ...
  methods: {
    ...
    zhengl(data) {
      ...
      if (nodeList != undefined) {
        ...
        var NodesText2 = nodeList.filter(node => node.group === '2' && node.type === 1);
        ...
        NodesText2.forEach((node, index) => {
          nodes.push({
            ...
            text: node.name,
            level: 1,
            x: startText2 + index * 200, // 水平等距排列
            y: 3700,                 // leve7 的固定 Y 坐标
            fixed: true,            // 固定位置
            data: { ...node },
            width: 180,
            height: 180,
        ...
        var NodesText3 = nodeList.filter(node => node.group === '3' && node.type === 1);
        ...
        NodesText3.forEach((node, index) => {
          nodes.push({
            ...
            text: node.name,
            level: 1,
            x: startText3 + index * 200, // 水平等距排列
            y: 3900,                 // leve7 的固定 Y 坐标
            fixed: true,            // 固定位置
            data: { ...node },
            width: 180,
            height: 180,
        ...
        var NodesText4 = nodeList.filter(node => node.group === '4' && node.type === 1);
        ...
        var NodesText5 = nodeList.filter(node => node.group === '5' && node.type === 1);
